...
public:
	...
	/**
		* @brief Estima la entrop�a aproximada (en bits) de una contrase�a.
		*        Basado en el tama�o del pool de caracteres usados y la longitud.
		* @param password Contrase�a a evaluar.
		* @return Bits de entrop�a estimados.
		*/
	double
		...
		if (hasLower) {
			poolSize += 26;  // 26 letras min�sculas.
		}
		if (hasUpper) {
			poolSize += 26;  // 26 letras may�sculas.
		}
		if (hasDigit) {
			poolSize += 10;  // 10 d�gitos (0-9).
		}
		if (hasSymbols) {
			poolSize += 32;  // Aproximadamente 32 s�mbolos comunes.
		}
		if (poolSize == 0) {
			std::cout << "No character types enabled for entropy estimation." << std::endl;
			return 0.0;  // Si no hay caracteres v�lidos, entrop�a es 0.
		}
		double entropy = password.size() * std::log2(static_cast<double>(poolSize));  // Entrop�a = log2(poolSize) * longitud.
		return entropy;  // Devuelve la entrop�a estimada en bits.
	}

	/**
		 * @brief Devuelve una calificaci�n cualitativa de la fortaleza de la contrase�a.
		 * @param password Contrase�a a calificar.
		 * @return Una cadena: "Muy d�bil", "D�bil", "Moderada", "Fuerte" o "Muy fuerte".
		 */
	std::string
		passwordStrength(const std::string& password) {
		double entropy = estimateEntropy(password);
		if (entropy < 28) {
			return "Muy d�bil";  // Entrop�a < 28 bits.
		}
		else if (entropy < 40) {
			return "D�bil";  // Entrop�a entre 28 y 40 bits.
		}
		else if (entropy < 60) {
			return "Moderada";  // Entrop�a entre 40 y 60 bits.
		}
		else if (entropy < 80) {
			return "Fuerte";  // Entrop�a entre 60 y 80 bits.
		}
		else {
			return "Muy fuerte";  // Entrop�a >= 80 bits.
		}
